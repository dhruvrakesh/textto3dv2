...
-- Step 3: Drop existing functions with changed return types
DROP FUNCTION IF EXISTS public.get_t3d_jobs(uuid);
DROP FUNCTION IF EXISTS public.get_t3d_prompts(uuid);
DROP FUNCTION IF EXISTS public.get_t3d_prompt_by_id(uuid);

-- Step 4: Create clean, single-instance RPC wrapper functions
...
-- Step 5: Recreate get_t3d_jobs function with correct integer return type for progress
CREATE FUNCTION public.get_t3d_jobs(p_user_id uuid)
RETURNS TABLE(
  id uuid, 
  prompt_id uuid, 
  user_id uuid, 
...
-- Step 6: Recreate get_t3d_prompts function
CREATE FUNCTION public.get_t3d_prompts(p_user_id uuid)
RETURNS TABLE(
  id uuid,
  user_id uuid,
  space text,
...
-- Step 7: Recreate get_t3d_prompt_by_id function
CREATE FUNCTION public.get_t3d_prompt_by_id(p_prompt_id uuid)
RETURNS TABLE(
  id uuid,
  user_id uuid,
  space text,
